...
const EventDetail: React.FC<{ event: BaariaEvent; onSelectProjection: (projection: ProgrammazioneItem) => void; onBack: () => void; }> = ({ event, onSelectProjection, onBack }) => {
    ...
    if (!details) {
        return (
            <div className="text-center py-10">
                ...
                <button onClick={onBack} className="mt-6 px-4 py-2 bg-gray-700 hover:bg-gray-600 text-white rounded-md flex items-center gap-2 transition-colors duration-200 shadow hover:shadow-lg mx-auto">
    ...
    return (
        <div className="space-y-8">
            <button onClick={onBack} className="mb-6 px-4 py-2 bg-gray-700 hover:bg-gray-600 text-white rounded-md flex items-center gap-2 transition-colors duration-200 shadow hover:shadow-lg">
                <ChevronLeft className="h-5 w-5" /> Torna Indietro
            </button>
            <div className="bg-gray-800 rounded-xl shadow-2xl overflow-hidden">
                    ...
                    <div className="p-6 md:p-8 md:w-3/5 xl:w-2/3">
                        ...
                        {details.trailer_video_url && (
                            <div className="mb-8">
                                ...
                                <div className="aspect-video rounded-lg overflow-hidden shadow-lg"><video controls width="100%" src={details.trailer_video_url} className="w-full h-full bg-black">Il tuo browser non supporta il tag video.</video></div>
                            </div>
                        )}
                    </div>
                </div>
                ...
                {details.galleria_immagini && details.galleria_immagini.length > 0 && (
                    <div className="p-6 md:p-8 border-t border-gray-700/50">
                        <h3 className="text-2xl font-semibold text-gray-100 mb-4 flex items-center"><ImageIcon size={24} className="mr-2 text-[#ebdaa8]" />Galleria</h3>
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                            {details.galleria_immagini.map((image, index) => (
                                <a key={index} href={image.url} target="_blank" rel="noopener noreferrer" className="block rounded-lg overflow-hidden aspect-w-1 aspect-h-1 hover:opacity-80 transition-opacity shadow-md">
                                    <img src={image.url} alt={image.alt || `Immagine ${index + 1}`} className="w-full h-full object-cover" onError={(e) => (e.currentTarget.src = "https://placehold.co/200x200/3A3A3A/EAA949?text=IMG")} />
                                </a>
                            ))}
                        </div>
                    </div>
                ...
                <div className="p-6 md:p-8 border-t border-gray-700/50">
                    ...
                    ) : ( <p className="text-gray-400 italic">Nessuna proiezione programmata.</p> )}
                </div>
            </div>
        </div>
    );
