  ...
  onCheckoutSuccess: (customerEmail: string) => void;
}> = ({ eventData, projectionData, quantity, onCheckoutSuccess }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (validateForm()) {
            ...
            setErrors(prev => ({ ...prev, submit: "" }));
            try {
                const response = await requestBookingWithPdf({ 
                    ...customerInfo, 
    ...
    const displayEvents = isEventGroup ? selectedEventGroup.map(item => item.event) : [eventData];
                    ...
                    <div className="flex items-start">
                        ...
                        <div>
                            {displayEvents.map(event => (
                                <p key={event.id} className="font-medium text-gray-100">{event.title.rendered}</p>
                            ))}
                            {isEventGroup && <p className="font-medium text-gray-100 mt-2">Programma Cortometraggi: ATOLLI</p>}
                        </div>
                    </div>
